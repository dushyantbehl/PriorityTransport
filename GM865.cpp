...
GM865::GM865(HardwareSerial *modemPort, byte onOffPin, char *pin) {
  state = STATE_NONE;
  modem = modemPort;
  modem->begin(19200);
...
boolean GM865::isInitialized() {
  return (state & STATE_INITIALIZED);
}


boolean GM865::isRegistered() {
  return (state & STATE_REGISTERED);
}


boolean GM865::isPosFixed() {
...
byte GM865::requestModem(const char *command, uint16_t timeout, boolean check, char *buf) {
...
byte GM865::getsTimeout(char *buf, uint16_t timeout) {
  byte count = 0;
  long timeIsOut = 0;
  char c;
  *buf = 0;
...
void GM865::sendSMS(char *number, char *message) {
  char buf[BUF_LENGTH];
...
boolean GM865::openHTTP(char *domain) {
  char buf[BUF_LENGTH];
...
void GM865::send(char *buf) {
    Serial.print(buf);
    modem->print(buf);
}


char *GM865::receive(char *buf) {
  getsTimeout(buf, 1000);
  return buf;
}


void GM865::warmStartGPS() {
  char buf[BUF_LENGTH];
...
Position GM865::requestGPS(void) {
  char buf[150];
...
Position GM865::getLastPosition() {
  return actPosition;
}
 ...
 */
void GM865::parseGPS(char *gpsMsg, Position *pos) {
 ...
 */
char *GM865::skip(char *str, char match) {
  uint8_t c = 0;
  while (true) {
    c = *str++;
    if ((c == match) || (c == '\0')) {
 ...
 * given delimiter.
 */
char *GM865::readToken(char *str, char *buf, char delimiter) {
  uint8_t c = 0;
  while (true) {
    c = *str++;
    if ((c == delimiter) || (c == '\0')) {
 ...
 */
void GM865::parsePosition(Position *pos, char *lat_str, char *lon_str, char *alt_str) {
  char buf[10];
 ...
 */
void GM865::parseDegrees(char *str, int *degree, long *minutes) {
  char buf[6];
  uint8_t c = 0;
  uint8_t i = 0;
  char *tmp_str;
